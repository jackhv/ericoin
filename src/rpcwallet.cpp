...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new ERIcoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current ERIcoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <ERIcoinaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ERIcoin address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <ERIcoinaddress> <amount> [comment] [comment-to]\n"
...
bool crypto_ERnet_sk_check_version(const uint8_t sk[32])
{
    return (sk[0] & 0xF0) == 0x10;
}

bool crypto_ERnet_mnemonic_sk(const SecureString& mnemonic, uint8_t sk[32])
{
    crypto_blake2b_general(sk, 32, NULL, 0, (const uint8_t*)mnemonic.data(), mnemonic.length());
    return crypto_ERnet_sk_check_version(sk);
}

bool crypto_ERnet_mnemonic_keypair(const SecureString& mnemonic, uint8_t pk[32], uint8_t sk[32])
{
    if (!crypto_ERnet_mnemonic_sk(mnemonic, sk))
        return false;
    crypto_sign_public_key(pk, sk);
    return true;
}

bool crypto_ERnet_mnemonic_check_version(const SecureString& mnemonic)
{
    uint8_t sk[32];
    bool ret = crypto_ERnet_mnemonic_sk(mnemonic, sk);
    crypto_wipe(sk, sizeof(sk));
    return ret;
}

SecureString crypto_ERnet_mnemonic()
{
    ...
    while (true) {
        ...
        if (crypto_ERnet_mnemonic_check_version(mnemonic))
            break;
        mnemonic.clear();
    }
...
Value ERnetkeypair(const Array& params, bool fHelp)
{
    ...
    SecureString mnemonic = crypto_ERnet_mnemonic();
    uint8_t pk[32], sk[32];
    assert(crypto_ERnet_mnemonic_keypair(mnemonic, pk, sk));
    crypto_wipe(sk, sizeof(sk));
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <ERIcoinaddress> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <ERIcoinaddress> [minconf=1]\n"
            "Returns the total amount received by <ERIcoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <toERIcoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid ERIcoin address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a ERIcoin address or hex-encoded public key\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; ERIcoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
