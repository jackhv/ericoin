...
{
    // Convert ERIcoin:// to ERIcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ERIcoin://"))
    {
        uri.replace(0, 12, "ERIcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ERIcoin.lnk";
}
...
{
    return GetAutostartDir() / "ERIcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ERIcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ERIcoin-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetERThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QPushButton    { background: rgb(121,158,226); color: rgb(21,21,21); }"
